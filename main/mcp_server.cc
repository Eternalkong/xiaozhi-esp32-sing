 ...
 */
 ...
 void McpServer::AddCommonTools() {
     ...
     auto sing = board.GetSing();
     if (sing) {
         AddTool("self.sing.play_song",
             "唱指定的歌曲。当用户要求唱歌时使用此工具，会自动获取歌曲详情并开始流式播放。\n"
             "参数:\n"
             "  `song_name`: 要唱的歌曲名称（必需）。\n"
             "  `artist_name`: 要唱的歌曲艺术家名称（可选，默认为空字符串）。\n"
             "返回:\n"
             "  播放状态信息，不需确认，立刻播放歌曲。",
             PropertyList({
                 Property("song_name", kPropertyTypeString),//歌曲名称（必需）
             ...
             [sing](const PropertyList& properties) -> ReturnValue {
                 auto song_name = properties["song_name"].value<std::string>();
                 auto artist_name = properties["artist_name"].value<std::string>();
                 
                 if (!sing->Download(song_name, artist_name)) {
                     return "{\"success\": false, \"message\": \"获取音乐资源失败\"}";
                 }
                 auto download_result = sing->GetDownloadResult();
                 ESP_LOGI(TAG, "Sing details result: %s", download_result.c_str());
                 return "{\"success\": true, \"message\": \"歌曲开始播放\"}";
             });
     }
 ...
 void McpServer::ParseMessage(const cJSON* json) {
     ...
     if (method_str == "initialize") {
        ...
        const cJSON* args_obj = nullptr;
        cJSON* parsed_args = nullptr;
        if (tool_arguments == nullptr) {
            args_obj = nullptr;
        } else if (cJSON_IsObject(tool_arguments)) {
            args_obj = tool_arguments;
        } else if (cJSON_IsString(tool_arguments)) {
            // 兼容一些服务端会将 arguments 作为字符串化 JSON 发送的情况
            parsed_args = cJSON_Parse(tool_arguments->valuestring);
            if (parsed_args != nullptr && cJSON_IsObject(parsed_args)) {
                args_obj = parsed_args;
            } else {
                // 回退策略：如果该工具只有一个必需的字符串参数，则将该字符串直接映射到该参数
                auto tool_iter = std::find_if(tools_.begin(), tools_.end(), 
                                              [tool_name](const McpTool* tool) { 
                                                  return tool->name() == std::string(tool_name->valuestring); 
                                              });
                if (tool_iter != tools_.end()) {
                    const auto& props = (*tool_iter)->properties();
                    // 统计必需参数数量与类型
                    int required_count = 0;
                    std::string required_string_name;
                    for (auto it = const_cast<PropertyList&>(props).begin(); it != const_cast<PropertyList&>(props).end(); ++it) {
                        const Property& p = *it;
                        if (!p.has_default_value()) {
                            required_count++;
                            if (p.type() == kPropertyTypeString) {
                                required_string_name = p.name();
                            }
                        }
                    }
                    if (required_count == 1 && !required_string_name.empty()) {
                        // 创建一个对象，将字符串作为唯一必需的字符串参数值
                        parsed_args = cJSON_CreateObject();
                        cJSON_AddStringToObject(parsed_args, required_string_name.c_str(), tool_arguments->valuestring);
                        args_obj = parsed_args;
                        ESP_LOGW(TAG, "tools/call: Fallback mapped raw string to arg '%s' for tool %s", required_string_name.c_str(), tool_name->valuestring);
                    } else {
                        ESP_LOGE(TAG, "tools/call: Invalid arguments (string not JSON object): %s", tool_arguments->valuestring);
            ...
            ESP_LOGE(TAG, "tools/call: Invalid arguments type");
            ReplyError(id_int, "Invalid arguments");
            return;
        }
         ...
         DoToolCall(id_int, std::string(tool_name->valuestring), args_obj, stack_size ? stack_size->valueint : DEFAULT_TOOLCALL_STACK_SIZE);
         if (parsed_args) cJSON_Delete(parsed_args);
     } else {
